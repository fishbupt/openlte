    ...
    uint8** ie_ptr)
{
  ...
  if (mobile_id != NULL && ie_ptr != NULL) {
    if (LIBLTE_MME_MOBILE_ID_TYPE_IMSI == mobile_id->type_of_id || LIBLTE_MME_MOBILE_ID_TYPE_IMEI == mobile_id->type_of_id) {
      ...
      *ie_ptr += 1;
      **ie_ptr = ((mobile_id->tmgi.mbms_session_id_ind << 5) | (mobile_id->tmgi.mcc_mnc_ind << 4) | mobile_id->type_of_id);
      *ie_ptr += 1;
    ...
    LIBLTE_MME_MOBILE_ID_STRUCT* mobile_id)
{
  ...
  if (ie_ptr != NULL && mobile_id != NULL) {
    ...
    *ie_ptr += 1;
    ...
    if (LIBLTE_MME_MOBILE_ID_TYPE_IMSI == mobile_id->type_of_id || LIBLTE_MME_MOBILE_ID_TYPE_IMEI == mobile_id->type_of_id || LIBLTE_MME_MOBILE_ID_TYPE_IMEISV == mobile_id->type_of_id) {
      if (LIBLTE_MME_MOBILE_ID_TYPE_IMSI == mobile_id->type_of_id) {
        id = mobile_id->imsi;
        odd = true;
    ...
    uint8** ie_ptr)
{
  ...
  if (ms_cm3 != NULL && ie_ptr != NULL) {
    ...
    if (ms_cm3->multi_band_support.pgsm_support && (ms_cm3->multi_band_support.e_rgsm_support || ms_cm3->multi_band_support.gsm_1800_support)) {
    ...
    } else if (ms_cm3->multi_band_support.pgsm_support || ms_cm3->multi_band_support.e_rgsm_support || ms_cm3->multi_band_support.gsm_1800_support) {
    ...
    LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_STRUCT* additional_update_type)
{
  ...
  if (ie_ptr != NULL && additional_update_type != NULL) {
    additional_update_type->autv = (LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_ENUM)((**ie_ptr) & 0x01);
    additional_update_type->saf = ((**ie_ptr >> 1) & 0x01);
    additional_update_type->pnb_ciot = (LIBLTE_MME_PREFERRED_CIOT_NETWORK_ENUM)((**ie_ptr >> 2) & 0x03);
    ...
    uint8** ie_ptr)
{
  ...
  if (eps_nfs != NULL && ie_ptr != NULL) {
    ...
    (*ie_ptr)[1] |= eps_nfs->erwo_pdn << 6;
    (*ie_ptr)[1] |= eps_nfs->cp_ciot << 7;
    (*ie_ptr)[2] = eps_nfs->up_ciot;
    (*ie_ptr)[2] |= eps_nfs->s1_u << 1;
    (*ie_ptr)[2] |= eps_nfs->hc_cp_ciot << 2;
    *ie_ptr += 3;
    ...
    LIBLTE_MME_EPS_NETWORK_FEATURE_SUPPORT_STRUCT* eps_nfs)
{
  ...
  if (ie_ptr != NULL && eps_nfs != NULL) {
    ...
    eps_nfs->erwo_pdn = ((*ie_ptr)[1] >> 6) & 0x01;
    eps_nfs->cp_ciot = ((*ie_ptr)[1] >> 7) & 0x01;
    if (length > 1) {
      eps_nfs->up_ciot = (*ie_ptr)[2] & 0x01;
      eps_nfs->s1_u = ((*ie_ptr)[2] >> 1) & 0x01;
      eps_nfs->hc_cp_ciot = ((*ie_ptr)[2] >> 2) & 0x01;
      *ie_ptr += 3;
    } else {
      *ie_ptr += 2;
    }
    ...
    uint8** ie_ptr)
{
  ...
  if (net_name != NULL && ie_ptr != NULL) {
    ...
    for (i = 0; i < net_name->name.size(); i++) {
      if (char_str[i] == 0x0A || char_str[i] == 0x0D || (char_str[i] >= 0x20 && char_str[i] <= 0x3F) || (char_str[i] >= 0x41 && char_str[i] <= 0x5A) || (char_str[i] >= 0x61 && char_str[i] <= 0x7A)) {
        switch (bit_offset) {
        case 0:
    ...
    LIBLTE_MME_NETWORK_NAME_STRUCT* net_name)
{
  ...
  if (ie_ptr != NULL && net_name != NULL) {
    ...
    while (byte_offset < N_bytes) {
      ...
      if (tmp_char == 0x0A || tmp_char == 0x0D || (tmp_char >= 0x20 && tmp_char <= 0x3F) || (tmp_char >= 0x41 && tmp_char <= 0x5A) || (tmp_char >= 0x61 && tmp_char <= 0x7A)) {
    ...
    uint8** ie_ptr)
{
  ...
  if (ue_network_cap != NULL && ie_ptr != NULL) {
    if (ue_network_cap->ciot_present) {
      **ie_ptr = 7;
    } else if (ue_network_cap->uea_present && (ue_network_cap->ucs2_present || ue_network_cap->uia_present) && (ue_network_cap->lpp_present || ue_network_cap->lcs_present || ue_network_cap->onexsrvcc_present || ue_network_cap->nf_present)) {
      **ie_ptr = 5;
    ...
    *ie_ptr += 1;
    ...
    if (ue_network_cap->lpp_present || ue_network_cap->lcs_present || ue_network_cap->onexsrvcc_present || ue_network_cap->nf_present) {
    ...
    if (ue_network_cap->ciot_present) {
      **ie_ptr = ue_network_cap->cp_ciot << 2;
      **ie_ptr |= ue_network_cap->up_ciot << 3;
      **ie_ptr |= ue_network_cap->s1_u << 4;
      **ie_ptr |= ue_network_cap->erwo_pdn << 5;
      **ie_ptr |= ue_network_cap->hc_cp_ciot << 6;
      *ie_ptr += 1;
      **ie_ptr = ue_network_cap->multiple_drb;
      *ie_ptr += 1;
    }
    ...
    LIBLTE_MME_UE_NETWORK_CAPABILITY_STRUCT* ue_network_cap)
{
  ...
  if (ie_ptr != NULL && ue_network_cap != NULL) {
    ...
    *ie_ptr += 1;
    ...
    if (length > 6) {
      ue_network_cap->ciot_present = true;
      ue_network_cap->cp_ciot = (**ie_ptr >> 2) & 0x01;
      ue_network_cap->up_ciot = (**ie_ptr >> 3) & 0x01;
      ...
      ue_network_cap->up_ciot = false;
      ue_network_cap->s1_u = false;
      ue_network_cap->erwo_pdn = false;
      ue_network_cap->multiple_drb = false;
    }
    ...
    *msg_ptr = 0;
    ...
    if (attach_req->additional_update_type_present) {
      *msg_ptr = LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_IEI << 4;
      liblte_mme_pack_additional_update_type_ie(&attach_req->additional_update_type, &msg_ptr);
      msg_ptr++;
    }
    ...
    LIBLTE_MME_ATTACH_REQUEST_MSG_STRUCT* attach_req)
{
  ...
  if (msg != NULL && attach_req != NULL) {
    ...
    if ((LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_additional_update_type_ie(&msg_ptr, &attach_req->additional_update_type);
      msg_ptr++;
  ...
  if (ta_update_req != NULL && msg != NULL) {
    ...
    *msg_ptr = 0;
    ...
    if (ta_update_req->additional_update_type_present) {
      *msg_ptr = LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_IEI << 4;
      liblte_mme_pack_additional_update_type_ie(&ta_update_req->additional_update_type, &msg_ptr);
      msg_ptr++;
    }
    ...
    LIBLTE_MME_TRACKING_AREA_UPDATE_REQUEST_MSG_STRUCT* ta_update_req)
{
  ...
  if (msg != NULL && ta_update_req != NULL) {
    ...
    // Additional Update Type
    if ((LIBLTE_MME_ADDITIONAL_UPDATE_TYPE_IEI << 4) == (*msg_ptr & 0xF0)) {
      liblte_mme_unpack_additional_update_type_ie(&msg_ptr, &ta_update_req->additional_update_type);
      msg_ptr++;
